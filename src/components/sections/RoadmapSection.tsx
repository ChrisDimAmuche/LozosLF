    ...
    <section className="py-12 sm:py-20 bg-black/80">
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-4 sm:left-1/2 transform sm:-translate-x-1/2 w-1 sm:w-1 h-full bg-yellow-500/20" />
          ...
          <div className="space-y-16 sm:space-y-24">
            {sortedPhases.map((phase, index) => (
              <div key={phase.id} className={`relative flex ${index % 2 === 0 ? 'justify-start' : 'justify-end'} sm:flex`}>
                ...
                {/* Content */}
                <div className={`w-full sm:w-5/12 ml-12 sm:ml-0 ${index % 2 === 0 ? 'sm:pr-12 lg:pr-16' : 'sm:pl-12 lg:pl-16'}`}>
                <div className="bg-white/5 backdrop-blur-lg rounded-xl p-3 sm:p-4 md:p-6 lg:p-8 border border-white/10">
                    <h3 className="text-base sm:text-lg md:text-xl font-bold text-white mb-3 sm:mb-4 md:mb-6">{phase.title}</h3>
                    <ul className="space-y-2 sm:space-y-3 md:space-y-4">
                      {phase.items.map((item, itemIndex) => (
                        <li key={itemIndex} className="flex items-start space-x-3">
                          <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500 flex-shrink-0 mt-1" />
